...
to setup-mind [ vision ]
  ...
  ls:let my-land-mobility land-mobility
  ls:let my-land-food-preference land-food-preference
  ls:let my-eye-size eye-size
  ls:let zero (0 - pycor)

  ls:let egff energy-gain-from-food

  ls:let my-breed (word breed)

  ls:let simulation_type simulation-mode
  ls:let er eat-radius
  ...
  ls:ask 0 [
    ...
    set food-coords fdcs
    set fish-coords fics
    set ego-breed my-breed
    set init-energy my-energy
    set init-xcor my-xcor
    set init-ycor my-ycor
    set init-heading my-heading
    set energy-gain-from-food egff
    set vision v
    set eye-cost ec

    set eat_radius er
    set ego-land-mobility my-land-mobility
    set ego-land-food-preference my-land-food-preference
    set ego-eye-size my-eye-size
    set zero-ycor zero
    set simulation_mode simulation_type
    init-setup
  ]
end
...
to reproduce
  if ( random 100 < reproduction-rate ) and ( energy > energy-for-reproduction ) [
    ...
    hatch 1 [
      ...
      set sim-l mutate-sim sim-l 1 0
      set sim-n mutate-sim sim-n 1 0
      set color scale-color red land-mobility 4 -2.5
...
"" ""
PENS
"default" 1.0 0 -2674135 true "" "plot (avg-sim-l (fish with [ land-mobility >= 0 ])) * (avg-sim-n (fish with [ land-mobility >= 0 ]))"
"pen-1" 1.0 0 -13345367 true "" "plot (avg-sim-l (fish with [ land-mobility < 0 ])) * (avg-sim-n (fish with [ land-mobility < 0 ]))"
...
simulation-mode
simulation-mode
"Random Food in Radius" "Random Turn by 30 Degrees" "Random Direction" "Random Cardinal Direction"
0
...
<experiments>
  ...
  <experiment name="Cog-Cost Tests" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="25000"/>
    <metric>(avg-sim-l (fish with [ land-mobility &gt;= 0 ]))</metric>
    <metric>(avg-sim-n (fish with [ land-mobility &gt;= 0 ]))</metric>
    <metric>(avg-sim-l (fish with [ land-mobility &lt; 0 ]))</metric>
    <metric>(avg-sim-n (fish with [ land-mobility &lt; 0 ]))</metric>
    <enumeratedValueSet variable="initial-number-fish">
      <value value="50"/>
    ...
    <enumeratedValueSet variable="initial-eye-size">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="eye-cost">
      <value value="9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="eat-radius">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="reproduction-rate">
      <value value="35"/>
    ...
    <enumeratedValueSet variable="simulation-mode">
      <value value="&quot;Random Food in Radius&quot;"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
@#$#@#$#@
